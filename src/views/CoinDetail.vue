    ...
    <template v-if="!isLoading">
      ...
      <table>
        <tr v-for="m in markets" :key="`${m.exchangeId}-${m.priceUsd}`" class="border-b">
          ...
          <td>
            <px-button :isLoading="m.isLoading || false" v-if="!m.url" @custom-click="getWebSite(m)">
...
export default {
  ...
  computed: {
    min() {
      return Math.min(
        ...this.history.map((hr) => parseFloat(hr.priceUsd).toFixed(2))
      );
    },
    ...
    getCoin() {
      ...
      Promise.all([api.getAsset(id), api.getAssetHistory(id), api.getMarkets(id)])
        .then(([asset, history, markets]) => {
          (this.asset = asset), (this.history = history), (this.markets = markets);
        }
      )
